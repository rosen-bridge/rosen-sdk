 ...
 * @param rawHexData
 */
export const parseRosenData = (rawHexData: string): MinimalOnChainRosenData => {
  // parse toChain
  const toChainHex = rawHexData.slice(0, 2);
  ...
  // parse bridgeFee
  const bridgeFeeHex = rawHexData.slice(2, 18);
  const bridgeFee = BigInt('0x' + bridgeFeeHex).toString();

  // parse networkFee
  const networkFeeHex = rawHexData.slice(18, 34);
  const networkFee = BigInt('0x' + networkFeeHex).toString();

  // parse toAddress
  const addressLengthCode = rawHexData.slice(34, 36);
  const addressHex = rawHexData.slice(
    36,
